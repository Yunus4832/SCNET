...
实施记录：内容主页面已增加独立仓库入口，仓库页面只调用共享配置服务，支持列表、添加、编辑、删除、启用/禁用、
确定性排序和连接测试；配置变更不会操作缓存、Profile 或安装资产。无仓库、非法/重复地址、禁用状态和连接测试结果均有
四语言文案，LanguageTool 校验通过；页面资源通过 XML 校验，Survivalcraft 构建及全量 331 项测试通过。当前 Linux 执行环境
没有可用视频设备，页面视觉与设备交互不在本阶段虚报通过，统一保留到阶段 7 的 Windows、Linux、Android 人工验收。

- [x] 在内容主页面增加独立“内容仓库”入口。
- [x] 实现仓库列表、添加、编辑、删除、启用、排序和测试连接交互。
- [x] 提供独立仓库管理入口；在线内容和 Mod 管理页不混入仓库配置操作，旧地址输入分别归属阶段 5–6。
- [x] 为无仓库、地址非法、重复仓库、测试失败和部分仓库禁用提供明确文案。
- [x] 使用 LanguageTool 增加并验证本期多语言键。

门禁：用户不进入 Mod 管理页即可完成全部仓库配置；仓库编辑不影响缓存包、Profile 和安装资产。

### 阶段 5：重构统一在线内容与版本 UI

实施记录：旧单仓库 `ContentServerScreen` 已由统一在线内容页取代。主列表按
`ContentPackageType + Identifier` 合并分页结果，支持搜索、类型、仓库、缓存/Profile/当前会话/冲突/缺失状态筛选；本地缓存
只标记仓库目录中的精确 hash，不会生成无来源的在线条目。选择内容后，底部向上展开的版本选择组件按各来源 `contentId` 有界读取
完整版本历史，合并同 hash 来源并保留同版本异 hash 冲突；下载使用自动来源回退并统一进入内容寻址缓存，Mod 不修改 Profile，
非 Mod 复用现有安装流程。完整版本历史在页面内最多缓存 32 个内容、有效期 5 分钟；部分失败结果不缓存，手动刷新立即失效全部
版本缓存。页面离开会取消请求，精确导航会跨页查找。聚合历史和 UI 状态新增 5 项测试，全量 336 项测试通过；设备视觉交互仍归
阶段 7 验收。

- [x] 将在线内容主列表改为按 ContentType + Identifier 聚合，并接入搜索、类型、仓库和状态筛选。
- [x] 删除在线内容页地址输入、仓库管理跳转及单地址客户端构造，使用共享仓库与目录服务。
- [x] 在在线内容页选中内容后使用向上展开的版本单选组件提供完整历史版本，不再切换到独立版本页。
- [x] 版本选择状态综合缓存、Profile 引用、当前会话、冲突和来源信息，并按精确 hash 执行下载。
- [x] 所有类型通过统一下载服务进入缓存；Mod 下载后不修改 Profile，非 Mod 下载后可发起已有安装流程。
- [x] 完成真实分页/加载更多、取消、重试、离线缓存状态和返回后状态刷新。
- [x] 为同版本多来源、同版本冲突、旧版本定位和精确筛选导航增加 UI 状态测试。
...
和当前 Runtime，保留同版本异 hash 独立条目并显示缓存、缺失及各引用作用域。Mod 管理页不提供在线内容或仓库跳转，远程浏览与
下载只从独立在线内容页进入。Profile 编辑、FilePicker 导入/导出和引用保护删除路径保留；本地内容包页静态审查确认不引用远程
目录、版本或仓库服务。旧远程 UI 模型和语言键已删除，`Settings.ContentServerUrl` 也从反射设置模型移除，不提供旧字段读取。
精确并集新增 3 项测试，全量 339 项测试通过；设备交互仍归阶段 7 验收。
...
- [x] 移除“在仓库中查找”和顶部在线内容跳转，保持 Mod 管理与远程下载 UI 完全解耦。
