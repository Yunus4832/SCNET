...
- [x] 提供供在线内容及 Mod 管理使用的仓库管理导航入口；在线内容和 Mod 管理页的旧地址输入分别归属阶段 5–6。
...
实施记录：旧单仓库 `ContentServerScreen` 已由统一在线内容页和版本详情页取代。主列表按
`ContentPackageType + Identifier` 合并分页结果，支持搜索、类型、仓库、缓存/Profile/当前会话/冲突/缺失状态筛选；本地缓存
作为无来源目录种子，因此全部仓库不可用时仍可浏览缓存。详情页按各来源 `contentId` 有界读取完整版本历史，合并同 hash 来源、
保留同版本异 hash 冲突，并明确显示最新、精确 hash、大小、缓存、Profile、当前会话、缺失、来源和部分失败状态。下载可使用
自动回退或显式来源，统一进入内容寻址缓存；Mod 不修改 Profile，非 Mod 复用现有安装流程。页面离开会取消请求，精确导航会
跨页查找，仓库管理返回后会刷新来源。聚合历史和 UI 状态新增 5 项测试，全量 336 项测试通过；设备视觉交互仍归阶段 7 验收。

- [x] 将在线内容主列表改为按 ContentType + Identifier 聚合，并接入搜索、类型、仓库和状态筛选。
- [x] 删除在线内容页地址输入及单地址客户端构造，使用仓库管理导航和共享服务。
- [x] 实现版本详情，默认突出最新、Profile 引用、缓存和当前会话版本，并提供完整历史版本列表。
- [x] 展示每个版本的 hash、大小、缓存、Profile 引用、来源、冲突和部分仓库失败状态。
