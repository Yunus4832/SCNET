...
20. GUI 必须先使用临时仓库完成缺失包下载，再创建 pending session 或重启；重启后只依赖已验证缓存。
    若仓库对客户端不可达或下载未完成，则不得进入重启阶段。
...
- `ModProfile` 删除 `ContentServerUrl`；每个本地 requirement 都持久化 `ModId + Version + PackageHash`。Mod 管理 UI 从已验证缓存包
  或在线目录的明确制品创建 requirement，不产生缺失 hash 的条目。
...
- [ ] 用仓库集合替换 `Settings.ContentServerUrl`，删除单地址字段及所有调用，不保留兼容读取。
...
门禁：无 UI 条件下可以可靠维护多个仓库及其客户端生命周期；全仓不存在运行中的 `Settings.ContentServerUrl`
单地址依赖，也不存在绕过 Factory/Pool 创建持久仓库客户端的调用。

### 阶段 2：建立目录聚合、版本与下载服务

- [ ] 扩展 ContentServer 客户端以分页查询内容聚合、版本历史和精确版本，不在 Screen 内拼 URL。
- [ ] 实现跨仓库目录聚合、SemVer 排序、同 hash 来源合并和同版本异 hash 冲突模型。
- [ ] 实现有界分页、取消、并发查询和部分仓库失败结果。
- [ ] 实现缓存优先的唯一下载入口，支持显式限定仓库及未指定仓库时的默认选择、同 hash 自动回退，并迁移现有下载调用。
- [ ] 增加聚合、冲突、部分失败、缓存命中零网络请求、显式单仓库、默认选择、下载回退和 hash 校验测试。
...
- [ ] 调整 GUI 联机编排，确保连接准备阶段先通过临时候选仓库完成下载，再创建 pending session 或重启。
...
- [ ] 删除任何要求用户在 Mod 管理或在线内容页临时输入服务器地址的交互。
...
门禁：在线内容是唯一手动远程下载入口；用户能够获取 Profile 所需旧版本，且不会把不同 hash 的同版本误认为同一包。
