...
internal sealed class LinuxFilePicker : IFilePicker
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Title)) arguments.Add($"--title={request.Title}");
        ...
        if (request.Extensions.Count > 0)
            arguments.Add($"--file-filter=Supported files | {string.Join(' ', request.Extensions.Select(extension => $"*{NormalizeExtension(extension)}"))}");
    ...
    public async Task<PickedSaveTarget?> PickSaveTargetAsync(FileSaveRequest request,
        CancellationToken cancellationToken = default)
    {
        var arguments = new List<string> { "--file-selection", "--save", "--confirm-overwrite",
    ...
    private static async Task<string?> RunAsync(IReadOnlyList<string> arguments, CancellationToken cancellationToken)
    {
        var executable = FindExecutable("zenity")
                         ?? throw new InvalidOperationException("The desktop file picker is unavailable because zenity is not installed.");
        var start = new ProcessStartInfo(executable)
        {
            UseShellExecute = false, RedirectStandardOutput = true, RedirectStandardError = true
        };
        foreach (var argument in arguments) start.ArgumentList.Add(argument);
        using var process = Process.Start(start) ?? throw new InvalidOperationException("Unable to start Linux file picker.");
