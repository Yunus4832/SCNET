...
public class ConnectionRequestPackage : IPackage
{
    ...
    public void Handle(ProjectNet projectNet, NetNode netNode, bool isServer)
    {
        ...
        if (projectNet.FindSubsystem<SubsystemGameInfo>(true)!.WorldSettings.IsNeedCommunityLogin)
        {
            ...
            if (ModsManager.Configs.TryGetValue(saveKey, out var v))
            {
                ...
                AcceptClient(token, netNode, connectionError, useExternalPassword, saveKey);
            }
            else
            {
        ...
        {
            AcceptClient(_token, netNode, connectionError);
        }
    }
    ...
    public void ReadData(PackageStreamReader reader)
    {
        ...
        _modInfos = new List<ModEntity>();
        for (var i = 0; i < count; i++)
        {
            var modEntity = new ModEntity();
            modEntity.ResourcesMd5 = reader.ReadString();
            modEntity.ModInfo = new ModInfo();
            modEntity.ModInfo.Name = reader.ReadString();
            modEntity.ModInfo.PackageName = reader.ReadString();
            modEntity.ModInfo.Version = reader.ReadString();
    ...
    private void AcceptClient(
        string token, NetNode netNode, StringBuilder connectionError,
        bool useExternalPassword = false, string saveKey = "")
    {
        var guid = Guid.Empty;
        ...
        var pwd2 = ProjectNet.SubsystemGameInfo.WorldSettings.Password;
        ...
        foreach (var info in _modInfos)
        {
            ...
            {
                if (!ProjectNet.IsReleaseVersion)
                {
                    continue;
                }
        ...
        if (_magic == _verifyMagic)
        {
            ...
            else if (_version != ProjectNet.ServerVersion)
            {
                connectionError.AppendLine("客户端和服务器版本不一致");
            }
            else if (ProjectNet.SubsystemPlayers.BlackPlayerGuidList.ContainsKey(token))
            {
                connectionError.AppendLine("你已被禁止加入服务器");
            }
            else if (netNode.ClientCount >= ProjectNet.SubsystemGameInfo.WorldSettings.MaxOnlinePlayerCount)
            {
            ...
            else
            {
                ...
                {
#if DEBUG
                    Log.Information($"通知Client[{c.ID}]新加入客户端{addClient.ID}");
#endif
