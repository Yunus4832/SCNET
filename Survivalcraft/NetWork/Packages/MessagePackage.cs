...
public class MessagePackage : IPackage
{
    ...
    public void Handle(ProjectNet projectNet, NetNode netNode, bool isServer)
    {
        switch (_messageMode)
        {
            case MessageMode.BaseMessage:
            {
                var gameWidgets = projectNet.FindSubsystem<SubsystemGameWidgets>(true)!;
                ...
                if (isServer)
                {
                    if (From != null)
                    {
                        ...
                        var flag = projectNet.FindSubsystem<SubsystemPlayers>(true)!.NoMsgPlayerGuidList
            ...
            case MessageMode.LargeMessage:
                if (!isServer)
                {
                    foreach (var player in projectNet.FindSubsystem<SubsystemPlayers>(true)!.PlayersData)
                    {
                        player.ComponentPlayer?.ComponentGui?.DisplayLargeMessage(_largeText, _smallText, _duration,
                            _delay);
                    }
                }
