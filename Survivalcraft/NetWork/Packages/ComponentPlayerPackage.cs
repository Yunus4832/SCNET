...
public class ComponentPlayerPackage : IPackage
{
    ...
    {
        _playerData = projectNet.FindSubsystem<SubsystemPlayers>(true)!
        ...
        switch (_type)
        {
            ...
            case PlayerAction.Hit:
                PlayerEvent(player =>
                {
                    projectNet.FindEntityById(_bodyId, entity =>
                    {
            ...
            case PlayerAction.DragDrop:
                ...
                PlayerEvent(player =>
                {
                    projectNet.FindSubsystem<SubsystemInventories>(true)!.FindInventoryById(_inventoryID, inventory =>
                    {
                        // 丢弃背包内的物品，不是活动栏的
