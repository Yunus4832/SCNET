...
public class PickablePackage : IPackage
{
    ...
    {
        var subsystemPickable = projectNet.FindSubsystem<SubsystemPickables>(true)!;
        switch (_type)
        {
            case PickType.Create:
            ...
            case PickType.Update:
                ...
                break;
            case PickType.Delete:
                subsystemPickable.PickableAction(_id, pick =>
                {
                    if (PlaySound)
                    {
                    ...
                    subsystemPickable.Pickables.Remove(pick);
                }, false);
                break;
            case PickType.RequestSync:
                var flag = subsystemPickable.PickableAction(_id,
                ...
                if (!isServer)
                {
                    foreach (var pickable in _pickables)
                    {
                        subsystemPickable.PickableAction(pickable.Id,
                            pick => { subsystemPickable.Pickables.Remove(pick); }, false);
                    }
                }
