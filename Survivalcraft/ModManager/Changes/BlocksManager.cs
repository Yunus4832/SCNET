...
public static class BlocksManager
{
    ...
    public static void Initialize()
    {
        ...
        int num;
        foreach (var block in ModsManager.ModList.SelectMany(entity => entity.Blocks))
        {
        ...
        ModsManager.HookAction("BlocksInitialized", modLoader =>
        {
            modLoader.BlocksInitialized();
            return false;
        });
    }
    ...
    public static void LoadBlocksData(string data)
    {
        ...
        for (var i = 1; i < blockDataArray.Length; i++)
        {
            ...
            {
                throw new InvalidOperationException(string.Format(LanguageControl.Get("BlocksManager", 2),
                    dataContentArray.Length != 0 ? dataContentArray[0] : LanguageControl.Unknown));
            }
            ...
            for (var j = 1; j < dataContentArray.Length; j++)
            {
                ...
                if (!fieldInfoDict.TryGetValue(text, out var value))
                {
                    throw new InvalidOperationException(
                        string.Format(LanguageControl.Get("BlocksManager", 5), text));
                }
                ...
                if (text2.StartsWith('#'))
                {
                    ...
                    obj = !string.IsNullOrEmpty(refTypeName)
                        ? (Blocks.FirstOrDefault(v => v.GetType().Name == refTypeName) ??
                           throw new InvalidOperationException(
                               string.Format(LanguageControl.Get("BlocksManager", 6),
    ...
    {
        if (!ModsManager.GetModEntity(modSpace, out var modEntity))
        {
            return null;
        }

        var block = modEntity!.Blocks.Find(p => p.GetType().Name == typeFullName);
        return block;
    }
