...
public static class CraftingRecipesManager
{
    ...
    public static void Initialize()
    {
        ...
        if ((XElement?)source1 != null)
        {
            ...
            foreach (var item in source2)
            {
                ...
                {
                    desc = LanguageControl.GetBlock(attributeValue,
                ...
                {
                    modLoader.OnCraftingRecipeDecode(_recipes, item, out flag);
                    return flag;
                });
                if (!flag)
                {
        ...
        if (craftingRecipe != null)
        {
            if (heatLevel < craftingRecipe.RequiredHeatLevel)
            {
                craftingRecipe = !(heatLevel > 0f)
                    ? new CraftingRecipe
                    {
                        Message = LanguageControl.Get(_typeName, 0)
                    }
                    : new CraftingRecipe
                    {
                        Message = LanguageControl.Get(_typeName, 1)
                    };
            }
            else if (playerLevel < craftingRecipe.RequiredPlayerLevel)
            {
                craftingRecipe = !(craftingRecipe.RequiredHeatLevel > 0f)
                    ? new CraftingRecipe
                    {
                        Message = string.Format(LanguageControl.Get(_typeName, 2), craftingRecipe.RequiredPlayerLevel)
                    }
                    : new CraftingRecipe
                    {
                        Message = string.Format(LanguageControl.Get(_typeName, 3), craftingRecipe.RequiredPlayerLevel)
                    };
            }
        }
    ...
    {
        var flag2 = false;
        var craftingIdR = string.Empty;
        int? dataR = null;
        ModsManager.HookAction("DecodeIngredient", modLoader =>
        {
            modLoader.DecodeIngredient(ingredient, out craftingIdR, out dataR, out flag2);
            return flag2;
        });
        if (flag2)
        {
            craftingId = craftingIdR;
            data = dataR;
            return;
        }
