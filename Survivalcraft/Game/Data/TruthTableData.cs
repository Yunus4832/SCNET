...
{
    private static readonly List<char> _hexChars =
    [
        '0',
        '1',
        '2',
    ...
    public void LoadString(string data)
    {
        for (var i = 0; i < 16; i++)
        {
            var num = i < data.Length ? _hexChars.IndexOf(char.ToUpperInvariant(data[i])) : 0;
            if (num < 0)
            {
                num = 0;
            }
    ...
    public string SaveString()
    {
        ...
        {
            var index = MathUtils.Clamp(d, 0, 15);
            stringBuilder.Append(_hexChars[index]);
        }
