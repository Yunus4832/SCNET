...
{
    private const string _typeName = nameof(SubsystemBowBlockBehavior);
        ...
        switch (state)
        {
            ...
            case AimState.Cancelled:
                if (CommonLib.WorkType != WorkType.Client)
                {
                    inventory.RemoveSlotItems(activeSlotIndex, 1);
                    inventory.AddSlotItems(activeSlotIndex,
                        Terrain.MakeBlockValue(num, 0, BowBlock.SetDraw(data, 0)), 1);
                }
    ...
    public override int GetProcessInventoryItemCapacity(IInventory inventory, int slotIndex, int value)
    {
        ...
        {
            return !BowBlock.GetArrowType(Terrain.ExtractData(inventory.GetSlotValue(slotIndex))).HasValue ? 1 : 0;
        }
        ...
        int processCount, out int processedValue, out int processedCount)
    {
        if (processCount == 1)
        {
            ...
            inventory.AddSlotItems(slotIndex, Terrain.MakeBlockValue(191, 0, BowBlock.SetArrowType(data, arrowType)),
                1);
        }
        else
        {
