...
public class DetonatorBlock : MountedElectricElementBlock
{
    ...
    {
        var model = ContentManager.Get<Model>("Models/Detonator");
        var detonatorMesh = model.FindMesh("Detonator")!;
        var boneAbsoluteTransform = BlockMesh.GetBoneAbsoluteTransform(
            detonatorMesh.ParentBone ??
            throw new InvalidOperationException("Required DetonatorMesh.ParentBone is null")
        );
        for (var i = 0; i < 6; i++)
        {
            var m = i >= 4
            ...
            BlockMeshesByData[i].AppendModelMeshPart(detonatorMesh.MeshParts[0],
        ...
        StandaloneBlockMesh.AppendModelMeshPart(detonatorMesh.MeshParts[0], boneAbsoluteTransform * m2,
    ...
    {
        BlocksManager.DrawMeshBlock(primitivesRenderer, StandaloneBlockMesh, color, 4f * size, ref matrix,
            environmentData);
    }
