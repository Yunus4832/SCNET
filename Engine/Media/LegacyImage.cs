...
public class LegacyImage
{
    ...
    public static IEnumerable<LegacyImage> GenerateMipmaps(LegacyImage image, int maxLevelsCount = int.MaxValue)
    {
        ...
        while (true)
        {
            ...
            var num3 = num / mipWidth;
            var num4 = num2 / mipHeight;
            if (num3 == 2 && num4 == 2)
            {
                var i = 0;
                var num5 = 0;
                for (; i < mipHeight; i++)
                {
                    ...
                    while (num7 < mipWidth)
                    {
                        var color = image.Pixels[num6];
                        var color2 = image.Pixels[num6 + 1];
                        var color3 = image.Pixels[num6 + num];
                        var color4 = image.Pixels[num6 + num + 1];
                        var r = (byte)((color.R + color2.R + color3.R + color4.R + 2) / 4);
                        var g = (byte)((color.G + color2.G + color3.G + color4.G + 2) / 4);
                        var b = (byte)((color.B + color2.B + color3.B + color4.B + 2) / 4);
                        var a = (byte)((color.A + color2.A + color3.A + color4.A + 2) / 4);
            ...
            else if (num3 == 2 && num4 == 1)
            {
                ...
                for (; j < mipHeight; j++)
                {
                    ...
                    {
                        var color5 = image.Pixels[num9];
                        var color6 = image.Pixels[num9 + 1];
                        var r2 = (byte)((color5.R + color6.R + 1) / 2);
                        var g2 = (byte)((color5.G + color6.G + 1) / 2);
                        var b2 = (byte)((color5.B + color6.B + 1) / 2);
                        var a2 = (byte)((color5.A + color6.A + 1) / 2);
            ...
            else
            {
                ...
                for (; k < mipHeight; k++)
                {
                    ...
                    {
                        var color7 = image.Pixels[num12];
                        var color8 = image.Pixels[num12 + num];
                        var r3 = (byte)((color7.R + color8.R + 1) / 2);
                        var g3 = (byte)((color7.G + color8.G + 1) / 2);
                        var b3 = (byte)((color7.B + color8.B + 1) / 2);
                        var a3 = (byte)((color7.A + color8.A + 1) / 2);
