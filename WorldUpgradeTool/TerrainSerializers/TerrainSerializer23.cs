...
public class TerrainSerializer23 : IDisposable
{
    ...
    {
        _storage = new RegionFileStorage(this);
        _storage.Open(directoryName, suffix);
    }
    ...
    {
        Utilities.Dispose(ref _storage!);
    }
    ...
    private bool LoadChunkData(TerrainChunk chunk)
    {
        lock (_lock)
        {
            ...
            try
            {
                var readDataByteCount = _storage.Load(chunk.Coords, _storageBuffer);
            ...
            catch (Exception e)
            {
                Log.Error(ExceptionManager.MakeFullErrorMessage(
                    string.Format("Error loading chunk ({0},{1},{2},{3}).", chunk.Coords.X, chunk.Coords.Y,
    ...
    /// </summary>
    /// <param name="chunk"> 需要保存的 Chunk 数据块对象 </param>
    /// <param name="regionHeight"> 保存 Region 时的区块高度，版本 2.3 默认 256 </param>
    private void SaveChunkData(TerrainChunk chunk, int regionHeight = 256)
    {
        lock (_lock)
        {
            _ = Time.RealTime;
            ...
            {
                // 先压缩获得 buffer，然后保存
                var size = CompressChunkData(chunk, _storageBuffer, regionHeight);
                _storage.Save(chunk.Coords, _storageBuffer, size);
            }
            catch (Exception e)
            {
                Log.Error(ExceptionManager.MakeFullErrorMessage(
    ...
    /// <param name="regionHeight">压缩的 区块高度，版本 2.3 默认 256 </param>
    /// <returns> 返回压缩后的大小 </returns>
    /// <exception cref="InvalidOperationException"></exception>
    private int CompressChunkData(TerrainChunk chunk, byte[] buffer, int regionHeight = 256)
    {
        // 当前缓冲区大小
        var bufferSize = 0;
        for (var i = 0; i < 16; i++)
        ...
        // 首先使用 RLE 算法压缩重复的数据, 版本 2.3 层高默认 256
        var value = -1; // 值
        var count = 0; // 值重复的次数
    ...
    private void DecompressChunkData(TerrainChunk chunk, byte[] buffer, int size)
    {
        ...
        // 层高 cellY 最终应该是区块的最大高度（版本 2.3 256），cellX 和 cellZ 此时应当归零，否则该数据块已损坏
    ...
    ///     存档存储类
    /// </summary>
    public class RegionFileStorage(TerrainSerializer23 terrainSerializer23) : IStorage
    {
        ...
        private TerrainSerializer23 _terrainSerializer = terrainSerializer23;
        ...
        {
            return string.Format("{0}/Region {1},{2}.dat", new object[] { _regionsDirectoryName, region.X, region.Y });
        }
        ...
        private static DirectoryEntry ReadDirectoryEntry(BinaryReader reader)
        {
            ...
            if (result.Size < 0 || result.Size > 2097152)
            {
